 ...
 */
function exc_theme_css_alter(&$css) {
...
function exc_theme_preprocess_order_template(&$vars) {
  $offer = excur_offer_load($vars['order']);
  $node = $vars['node'];

  $vars['form'] = drupal_get_form('excur_offer_order_form', $offer);
}

/**
 * Process variables for order-form.tpl.php
 */
function exc_theme_preprocess_order_form(&$vars) {
  global $language;

  $node = $vars['form']['#data']['node'];
  $offer = $vars['form']['#data']['offer'];
  ...
  if (!empty($node->field_image)) {
    $vars['image'] = theme('image_style', array(
        ...
        'attributes' => array(
          'class' => array('modal-pay-image-inner'),
        ),
      )
    );
  }

  $vars['time_image'] = theme('image', array(
      'path' => EXCUR_FRONT_THEME_PATH . '/images/time.png',
      'attributes' => array(
        'class' => array('icont'),
      ),
  ...
  $vars['calendar_image'] = theme('image', array(
      'path' => EXCUR_FRONT_THEME_PATH . '/images/calendar.png',
      'attributes' => array(
        'class' => array('iconcalnd'),
      ),
    )
  );

  $vars['place_image'] = theme('image', array(
      'path' => EXCUR_FRONT_THEME_PATH . '/images/place.png',
      'attributes' => array(
        'class' => array('iconplace', 'excur-contacts-iconplace', 'modal-lebel-icon'),
      ),
    )
  );

  $vars['close_image'] = theme('image', array(
  ...
  foreach ($tickets as $ticket_title => $value) {
    $vars['tickets'][$i]['title'] = $ticket_title . ' (' . $value['count'] . ')';
    $vars['tickets'][$i]['price'] = $value['price'] * $value['count'];
    $vars['tickets'][$i]['currency'] = excur_offer_user_currency();
    $i++;
  }
  ...
  $vars['offer'] = array(
    ...
    'id' => $offer->id,
    'price' => $total_price,
    'currency' => excur_offer_user_currency(),
    'offer' => $offer->offer,
    'date' => $offer->date,
    'duration' => $offer->duration,
  );

  if(!empty($offer->start_time)) {
    $vars['offer']['start_time'] = $offer->start_time;
  }

  if(!empty($offer->venue)) {
    $vars['offer']['venue'] = $offer->venue;
  }
...
function exc_theme_preprocess_pay_template(&$vars) {
  $offer = excur_offer_load($vars['off_id']);
